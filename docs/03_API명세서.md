...
| 비고령·정상(신뢰수취인 없음) | 이지훈 / 660-7788-990011 | 임의 미등록 계좌 입력 | 600,000 | 아니오 |
| 비고령·신뢰 수취인 즉시 완료 | 한소영 / 770-8899-001122 | 신협 / 823-119-004432 | 350,000 | 아니오 |
| 고령·신뢰 수취인(2번째 케이스) | 강필용 / 880-9900-112233 | 기업은행 / 204-778-330091 | 250,000 | 아니오 |
| 애매(구조화 의심, auto-signal 미도달) | 오순덕 / 990-0011-223344 | 케이뱅크 / 701-334-882210 | 2,000,000 | 아니오 |
...
위 6~10개는 데모용으로 손으로 만든 대표 케이스이고, 별도로 `backend/app/data/generate_test_accounts.py`가 정상/대포통장(즉시인출·분산입금·심야거래·복합) 6개 원형을 무작위 파라미터로 다수(기본 30명/30개) 생성해 `accounts_generated.py` + `account_transactions/GEN_*.csv`로 추가해둔다. 위험 점수는 계좌마다 미리 정해두지 않고 `account_analysis.py`가 매번 CSV에서 재계산하며, 스크립트 실행 로그에서 점수가 0~100 사이에 실제로 분포하는 것을 바로 확인할 수 있다(판정이 하드코딩 라벨이 아니라는 근거). 계좌 수를 더 늘리려면 스크립트의 `N_PER_ARCHETYPE`을 올리고 재실행하면 된다.
